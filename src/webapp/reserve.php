...
if (!isset($_SESSION['loggedIn']) || !$_SESSION['loggedIn']) {
    header("Location: reserve.html?error=" . urlencode("You must be logged in to reserve a slip."));
    exit;
}

// Only accept POST
if ($_SERVER["REQUEST_METHOD"] !== "POST" || !isset($_POST['slipId'])) {
    header("Location: reserve.html?error=" . urlencode("Invalid request."));
    exit;
}

// CSRF Protection
if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
    header("Location: reserve.html?error=" . urlencode("Invalid security token. Please try again."));
    exit;
}
...
if ($conn->connect_error) {
    header("Location: reserve.html?error=" . urlencode("Database connection failed."));
    exit;
}
...
if ($row = $result->fetch_assoc()) {
    if ($row['available']) {
        ...
        */
        ...
        $conn->close();
        header("Location: reserve.html?success=1");
        exit;
    } else {
        $conn->close();
        header("Location: reserve.html?error=" . urlencode("Sorry, this slip has already been reserved."));
        exit;
    }
} else {
    $conn->close();
    header("Location: reserve.html?error=" . urlencode("Slip not found."));
    exit;
}
?>
